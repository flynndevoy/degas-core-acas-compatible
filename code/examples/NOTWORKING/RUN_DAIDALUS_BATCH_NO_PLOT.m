% RUN_DAIDALUS_BATCH_NO_PLOT
% Run DAIDALUS encounters in a loop with no plotting.
...
outCsv = fullfile(outDir, ['sep_daidalus_' ts '.csv']);
...
fprintf('\n[daidalus] Running %d encounters\n', numel(encounterIds));

for encNum = encounterIds
    fprintf('[daidalus] encounter=%d\n', encNum);
...
if ~isempty(rows)
    ...
    fprintf('[daidalus] CSV: %s\n', outCsv);
else
    fprintf('[daidalus] No separation data recorded.\n');
end

fprintf('[daidalus] Done.\n');
